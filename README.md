...
- 📝 **جدید در v0.4.2**: Rich Markdown تلگرام، لینک‌های کوتاه شده، کد collapsible، پشتیبانی کامل RTL
...
├── package.json            # نسخه 0.4.2 + اسکریپت‌ها
├── VERSION                 # 0.4.2
├── LICENSE                 # MIT
...
### ویژگی‌های جدید ILIVIR3:

**شخصیت:**
- عاشق استفاده از blockquote برای زیبایی متن
- علاقه‌مند به Rich Markdown تلگرام
- حرفه‌ای با تمرکز بر ظاهر زیبا

**فرمت‌بندی:**
- پشتیبانی کامل RTL فارسی/عربی
- تبدیل تمام لینک‌ها به `<a href="url">متن-کوتاه</a>`
- کدها همیشه collapsible و قابل کپی: `<pre><code>` یا `<blockquote><code>`
- پرامپت‌ها تشخیص داده شده و با فرمت `<b>Label:</b>` + `<blockquote><pre><code>>` ارسال می‌شوند
- پاراگراف اول هرگز quote نمی‌شود (قلاب پست است)
- مراحل شماره‌دار در یک blockquote گروه‌بندی می‌شوند
- ایموجی هرگز اول پست قرار نمی‌گیرد

**کنترل بازنویسی:**
- Intensity فقط روی UI تأثیر دارد، هرگز باعث بازنویسی نمی‌شود
- درصد بازنویسی جداگانه کنترل می‌شود
- پیام‌های بلند (>4096 کاراکتر) هوشمندانه خلاصه می‌شوند

**قوانین:**
- 65 قانون غیرقابل نقض برای رفتار پایدار
- تشخیص و فرمت‌بندی خودکار لینک‌ها، کدها، پرامپت‌ها
- حذف آرتیفکت‌های HTML از خروجی
