...
export function buildSystemPrompt(
  ...
  parts.push(
    `# OUTPUT CONTRACT (mandatory)\n` +
      ...
      `- LINKS: CRITICAL. Preserve ALL links EXACTLY where they appear in the source. If the source has an INLINE link like "برای دانلود [اینجا](url) کلیک کنید", KEEP it inline — do NOT move it to a separate line, do NOT strip it from the word it's attached to. If the source has a link at the START of the post, keep it at the START. If the source has a link in the MIDDLE, keep it in the MIDDLE. NEVER relocate, consolidate, or restructure links. The link's POSITION in the text is SACRED — moving it breaks the post's meaning. Inline [text](url) links MUST stay inline. Bare URLs MUST stay where they are.\n` +
...
function describeEmojiLevel(level: number): string {
  ...
  if (level <= 30) {
    ...
    return (
      ...
      "Emojis are RARELY needed. Most posts should have ZERO emojis. Only use ONE emoji if it genuinely enhances a key point — never decorative. " +
      "NEVER add emojis before links. NEVER use the same emoji across multiple posts. " +
