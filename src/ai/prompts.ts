...
export function buildSystemPrompt(
  ...
  parts.push(
    `# OUTPUT CONTRACT (mandatory)\n` +
      ...
      `- Persian punctuation: comma (،), question mark (؟), semicolon (؛). Half-spaces (نیم‌فاصله) MANDATORY in ALL compound words. Examples: می‌رود ✓ (not میرود), می‌تواند ✓, همه‌ی ✓, همه‌ما ✓ (not همهما), این‌که ✓, آن‌که ✓, به‌طور ✓, در‌حال‌که ✓, چون‌که ✓, را‌هرچند ✓, نگه‌داری ✓. ALWAYS insert half-space (U+200C / ـ) between prefix (می، هیچ، این، آن) and word stem, and between noun and suffix (ها، ی). Proofread EVERY word for missing half-spaces before returning.\n` +
